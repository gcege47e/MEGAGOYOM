...
def start(message):
    ...
    if user and has_accepted_terms(user_id):
        # کاربر قبلاً ثبت نام کرده و قوانین را پذیرفته
        bot.send_message(user_id, f"{EMOJIS['success']} خوش برگشتی!", reply_markup=main_menu())
        user_states[user_id] = "main_menu"
    else:
        # کاربر جدید یا قبلاً قوانین را نپذیرفته
...
def handle_message(message):
    ...
    # مدیریت دکمه برگشت به صفحه اصلی
