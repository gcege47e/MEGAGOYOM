...
def handle_ad_add_photo_collect(user_id, text):
    if text == f"{EMOJIS['success']} تأیید":
        if user_data[user_id]['photos']:
            ...
            c.execute("UPDATE users SET score = score + 40 WHERE user_id = ?", (user_id,))
            ...
            bot.send_message(user_id, f"{EMOJIS['success']} آگهی با موفقیت ثبت شد! ۴۰ امتیاز به شما اضافه شد.", reply_markup=get_ad_menu())
            user_states[user_id] = "ad_menu"
        else:
