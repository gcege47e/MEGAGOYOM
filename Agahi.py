...
def handle_message(message):
    ...
    if text == f"{EMOJIS['back']} برگشت":
        if state in ["profile_menu", "profile_view", "profile_edit_name", "profile_edit_age", "profile_edit_gender"]:
            ...
            return
        elif state in ["ad_menu", "ad_add_category", "ad_add_subcategory", "ad_add_title", "ad_add_description", "ad_add_price", "ad_add_address", "ad_add_phone", "ad_add_photo_collect", "ad_view_category", "ad_view_subcategory", "ad_my_ads", "ad_view_result", "ad_search_title", "ad_search_min_price", "ad_search_max_price"]:
            bot.send_message(user_id, "بخش آگهی‌ها:", reply_markup=get_ad_menu())
            user_states[user_id] = "ad_menu"
            return
        elif state in ["edit_ad_menu", "edit_ad_title", "edit_ad_description", "edit_ad_price", "edit_ad_address", "edit_ad_phone", "edit_ad_photo_collect"]:
    ...
    if state == "awaiting_terms_acceptance":
            ...
            if user_data[user_id]['photos']:
                ...
                c.execute("INSERT INTO ads (user_id, category, subcategory, title, description, price, address, phone, photo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                          (user_id, user_data[user_id]['category'], user_data[user_id]['subcategory'], user_data[user_id]['title'],
                           ...
                           user_data[user_id]['phone'], photos))
        ...
        if text == "رد کردن":
            ...
            if ads:
                ...
                bot.send_message(user_id, f"{EMOJIS['error']} هیچ آگهی‌ای با این عنوان یافت نشد!")
                bot.send_message(user_id, "انتخاب کنید:", reply_markup=back_home_buttons())
                user_states[user_id] = "ad_view_result"
        else:
            try:
        ...
        try:
            ...
            if max_price >= min_price:
                ...
                conn = sqlite3.connect(DB_NAME)
                c = conn.cursor()
                c.execute("SELECT ads.*, users.numeric_id FROM ads JOIN users ON ads.user_id = users.user_id WHERE ads.title LIKE ? AND CAST(REPLACE(ads.price, ',', '') AS INTEGER) BETWEEN ? AND ?",
                         (f"%{search_title}%", min_price, max_price))
                ads = c.fetchall()
                conn.close()
                if ads:
                    for ad in ads:
                    ...
                    bot.send_message(user_id, f"{EMOJIS['error']} هیچ آگهی‌ای با این عنوان و رنج قیمت یافت نشد!")
                    bot.send_message(user_id, "انتخاب کنید:", reply_markup=back_home_buttons())
                    user_states[user_id] = "ad_view_result"
            else:
...
if __name__ == '__main__':
    import time
    time.sleep(1)  # تأخیر برای اطمینان از آماده شدن سرور
    set_webhook()
