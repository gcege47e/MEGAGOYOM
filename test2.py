...
TOKEN = "8134200098:AAGGapErG9F0ek0lAEdrI53E5gzPDcObTQM"
ADMIN_ID = 7385601641
...
def get_main_menu():
    ...
    keyboard.add("😂 جوک‌ها", "🏪 مغازه‌ها")
    keyboard.add("⭐ گویمی‌های برتر", "📌 گمشده‌ها")
    if ADMIN_ID:
...
def start_handler(message):
    ...
    if user:
        bot.send_message(user_id, "سلام دوباره ✨\nبه منوی اصلی خوش اومدی!", reply_markup=get_main_menu())
    else:
...
def save_profile_edits(message):
    ...
    conn.close()
    del user_states[user_id]
    bot.send_message(user_id, "✅ تغییرات ذخیره شد!", reply_markup=get_main_menu())
...
def create_lost_item():
    ...
    conn.commit()
    conn.close()
    return jsonify({'message': 'Lost item created'})
...
def create_joke():
    ...
    conn.commit()
    conn.close()
    return jsonify({'message': 'Joke submitted'})
...
def approve_joke(joke_id):
    ...
    conn.commit()
    conn.close()
    return jsonify({'message': 'Joke approved'})
...
<body>
  ...
  <script type="text/babel">
    ...
    function App() {
      ...
      useEffect(() => {
        // Mocked user_id from Telegram WebApp context
        axios.get('/api/user/7385601641').then(res => setUser(res.data)).catch(err => console.error(err));
      }, []);
        ...
        <BrowserRouter>
          <div className="min-h-screen bg-gray-100">
            ...
            <Routes>
              ...
              <Route path="/lost" element={<LostList search={search} />} />
              <Route path="/lost/:id" element={<LostDetails />} />
              <Route path="/profile" element={<Profile user={user} setUser={setUser} />} />
              <Route path="/jokes" element={<Jokes />} />
              <Route path="/admin" element={<AdminPanel />} />
              <Route path="/add-item" element={<AddItemForm categories={categories} />} />
            </Routes>
          </div>
        </BrowserRouter>
      );
    ...
    function Home({ search }) {
      ...
      useEffect(() => {
        axios.get('/api/ads').then(res => setAds(res.data)).catch(err => console.error(err));
      }, []);
      ...
      return (
        <div className="container mx-auto p-4">
          ...
          <Link to="/add-item" className="fixed bottom-4 left-4 custom-button">+ افزودن آگهی</Link>
        </div>
      );
    }
    ...
    function AdDetails() {
      ...
      useEffect(() => {
        axios.get(`/api/ads/${id}`).then(res => setAd(res.data)).catch(err => console.error(err));
      }, [id]);
    ...
    // Lost Items List
    function LostList({ search }) {
      const [items, setItems] = useState([]);

      useEffect(() => {
        axios.get('/api/lost').then(res => setItems(res.data)).catch(err => console.error(err));
      }, []);
    ...
    // Lost Item Details Page
    function LostDetails() {
      const { id } = useParams();
      const [item, setItem] = useState(null);

      useEffect(() => {
        axios.get(`/api/lost/${id}`).then(res => setItem(res.data)).catch(err => console.error(err));
      }, [id]);
    ...
    function Profile({ user, setUser }) {
      ...
      useEffect(() => {
        if (user) {
          ...
          axios.get(`/api/ads?user_id=${user.user_id}`).then(res => setAds(res.data)).catch(err => console.error(err));
          axios.get(`/api/lost?user_id=${user.user_id}`).then(res => setLostItems(res.data)).catch(err => console.error(err));
        }
      }, [user]);
      ...
      const handleSave = () => {
        axios.put(`/api/user/${user.user_id}`, editData).then(() => {
      ...
      const handleDelete = (type, id) => {
        if (type === 'ad') {
          axios.delete(`/api/ads/${id}`).then(() => {
            setAds(ads.filter(ad => ad.id !== id));
          }).catch(err => console.error(err));
        } else {
          axios.delete(`/api/lost/${id}`).then(() => {
    ...
    function Jokes() {
      ...
      useEffect(() => {
        axios.get('/api/jokes').then(res => setJokes(res.data)).catch(err => console.error(err));
      }, []);

      const handleSubmitJoke = () => {
        axios.post('/api/jokes', { user_id: 7385601641, text: newJoke }).then(() => {
          setNewJoke('');
    ...
    function AdminPanel() {
      ...
      useEffect(() => {
        axios.get('/api/jokes/pending').then(res => setJokes(res.data)).catch(err => console.error(err));
        axios.get('/api/ads').then(res => setAds(res.data)).catch(err => console.error(err));
      }, []);

      const handleJokeAction = (jokeId, action) => {
        if (action === 'approve') {
          axios.post(`/api/jokes/${jokeId}/approve`).then(() => {
            setJokes(jokes.filter(j => j.id !== jokeId));
          }).catch(err => console.error(err));
        } else {
          axios.delete(`/api/jokes/${jokeId}`).then(() => {
            setJokes(jokes.filter(j => j.id !== jokeId));
          }).catch(err => console.error(err));
        }
      };
      ...
      const handleAdDelete = (adId) => {
        axios.delete(`/api/ads/${adId}`).then(() => {
    ...
    function AddItemForm({ categories }) {
      ...
      useEffect(() => {
        if (id) {
          ...
          axios.get(endpoint).then(res => setFormData(res.data)).catch(err => console.error(err));
        }
      }, [id, type]);

      const handleSubmit = () => {
        const data = { ...formData, user_id: 7385601641 };
        if (id) {
          const endpoint = type === 'ad' ? `/api/ads/${id}` : `/api/lost/${id}`;
