...
def show_top_users(call):
    ...
    if not top_users:
        bot.send_message(call.message.chat.id, "هنوز هیچ کاربری امتیازی ندارد.")
        return
        
    rankings_text = "✨ **گویمی‌های برتر** ✨\n\n"
    for i, user in enumerate(top_users, 1):
        username, points = user
        rankings_text += f"{convert_fa_numbers(str(i))}. **{username}** - {convert_fa_numbers(str(points))} امتیاز ⭐\n"
    
    bot.send_message(call.message.chat.id, rankings_text, parse_mode='Markdown')
