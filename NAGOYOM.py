...
def handle_message(message):
    ...
    if state == "awaiting_name":
        ...
        if text == f"{EMOJIS['profile']} پروفایل":
            ...
            help_text += "• جستجو بر اساس آدرس یا نام مکان (جزئی یا کامل)\n"
            ...
            help_text += f"{EMOJIS['places']} دسته‌بندی مکان‌ها (خلاصه):\n"
            help_text += "🍽️ خوراکی و نوشیدنی: رستوران‌ها، کافه‌ها، سوپرمارکت‌ها و ...\n"
            help_text += "🛍️ خرید و فروش: پاساژها، فروشگاه‌های پوشاک، لوازم خانگی و ...\n"
            help_text += "✂️ زیبایی و آرایشی: آرایشگاه‌ها، سالن‌های زیبایی، تتو و ...\n"
            help_text += "🏥 درمان و سلامت: بیمارستان‌ها، کلینیک‌ها، داروخانه‌ها و ...\n"
            help_text += "⚽ ورزش و سرگرمی: باشگاه‌ها، استخرها، سینماها و ...\n"
            help_text += "🏨 اقامت و سفر: هتل‌ها، آژانس‌های مسافرتی، فرودگاه‌ها و ...\n"
            help_text += "🏛️ خدمات عمومی و اداری: بانک‌ها، ادارات پست، شهرداری‌ها و ...\n"
            help_text += "🚗 خدمات شهری و حمل‌ونقل: پمپ بنزین‌ها، تعمیرگاه‌ها، تاکسی‌ها و ...\n"
            help_text += "📚 آموزش و فرهنگ: مدارس، دانشگاه‌ها، آموزشگاه‌ها و ...\n"
            help_text += "🕌 مذهبی و معنوی: مساجد، حسینیه‌ها، مراکز خیریه و ...\n"
            help_text += "🌳 طبیعت و تفریح آزاد: پارک‌ها، باغ وحش‌ها، مسیرهای طبیعت‌گردی و ...\n"
            help_text += "💼 کسب‌وکار و حرفه‌ای: دفاتر کار، کارخانه‌ها، شرکت‌های تبلیغاتی و ...\n"
            help_text += "🧰 خدمات تخصصی و فنی: تعمیرگاه‌ها، خدمات برق، خدمات نظافتی و ...\n\n"
        ...
        if text == "مغازه های آدرس مورد نظر":
            bot.send_message(user_id, "آدرس مورد نظر یا محدوده دلخواه را وارد کنید:", reply_markup=back_button_only())
        ...
        if text.strip():
            ...
            bot.send_message(user_id, f"{EMOJIS['places']} آدرس مورد نظر را وارد کنید (یا 'رد کردن' برای جستجو بدون آدرس):", reply_markup=skip_button())
...
# Section 1: Report Feature

import sqlite3
from telebot import types

def add_report(user_id, text):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    now = datetime.utcnow().isoformat()
    c.execute("INSERT INTO reports (user_id, text, created_at) VALUES (?, ?, ?)", (user_id, text, now))
    conn.commit()
    conn.close()

@bot.message_handler(func=lambda m: m.text == '📢 گزارش')
def handle_report_start(message):
    user_id = message.chat.id
    bot.send_message(user_id, "لطفاً اگر گزارشی دارید بنویسید تا به ادمین ارسال شود:")
    user_states[user_id] = "report_message"

@bot.message_handler(func=lambda m: user_states.get(m.chat.id) == "report_message")
def handle_report_message(message):
    user_id = message.chat.id
    report_text = message.text.strip()
    user_data[user_id] = {"report": report_text}
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row("✅ تایید ارسال", "🔙 برگشت")
    bot.send_message(user_id, "آیا از ارسال این گزارش مطمئن هستید؟", reply_markup=kb)
    user_states[user_id] = "report_confirm"

@bot.message_handler(func=lambda m: user_states.get(m.chat.id) == "report_confirm")
def handle_report_confirm(message):
    user_id = message.chat.id
    if message.text == "✅ تایید ارسال":
        draft = user_data.get(user_id, {}).get("report", "")
        add_report(user_id, draft)
        profile = send_profile(user_id)
        admin_msg = f"📢 گزارش جدید:\n\n{draft}\n---\n{profile}"
        bot.send_message(ADMIN_ID, admin_msg)
        bot.send_message(user_id, "✅ گزارش شما ارسال شد و پیگیری خواهد شد.", reply_markup=main_menu(user_id))
        user_states[user_id] = "main_menu"
    else:
        bot.send_message(user_id, "بازگشت به منو اصلی:", reply_markup=main_menu(user_id))
        user_states[user_id] = "main_menu"

# Section 2: Admin Top Places Management

def update_votes(title, owner_id, count, score, add=True):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT places.place_id, COALESCE(places.rating_sum,0), COALESCE(places.rating_count,0) FROM places JOIN users ON places.user_id = users.user_id WHERE places.title = ? AND users.numeric_id = ?", (title, int(owner_id)))
    row = c.fetchone()
    if not row:
        conn.close()
        return False, "مکان یافت نشد."
    pid, rsum, rcount = row
    rsum = float(rsum or 0.0)
    rcount = int(rcount or 0)
    if add:
        rsum += int(count) * float(score)
        rcount += int(count)
    else:
        rsum -= int(count) * float(score)
        rcount = max(0, rcount - int(count))
        if rcount == 0:
            rsum = 0.0
    avg = (rsum / rcount) if rcount > 0 else 0.0
    if avg < 0.0: avg = 0.0
    if avg > 10.0: avg = 10.0
    c.execute("UPDATE places SET rating_sum = ?, rating_count = ? WHERE place_id = ?", (rsum, rcount, pid))
    conn.commit()
    conn.close()
    action = "اضافه شد" if add else "کسر شد"
    return True, f"{count} رای با امتیاز {score} برای مکان '{title}' {action}. میانگین جدید: {avg:.2f}"

@bot.message_handler(func=lambda m: m.text == '🏆 قرارگیری مکان برتر' and m.chat.id == ADMIN_ID)
def handle_admin_top_places(message):
    user_id = message.chat.id
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row("⬆️ اضافه کردن رای", "⬇️ کم کردن رای")
    kb.row("🔙 برگشت")
    bot.send_message(user_id, "مدیریت مکان‌های برتر:", reply_markup=kb)
