...
def handle_message(message):
    ...
    if state == "awaiting_name":
        ...
        if text == f"{EMOJIS['profile']} پروفایل":
            ...
            help_text += "• مشاهده مکان‌هایی با حداقل ۱۰۰ رای و میانگین امتیاز بالای ۸\n"
        ...
        if text == "جستجو دوباره":
            ...
            bot.send_message(user_id, "بsection مکان‌ها:", reply_markup=get_place_menu())
            user_states[user_id] = "place_menu"
        ...
        if text == f"{EMOJIS['edit']} ویرایش عنوان":
            ...
            bot.send_message(user_id, f"{EMOJIS['edit']} شماره تماس جدید را وارد کنید (شماره باید با 09 شروع شود و 11 رقم باشد или 'رد کردن' برای خالی کردن):", reply_markup=skip_button())
            ...
            bot.send_message(user_id, f"{EMOJIS['edit']} شیفت عصر جدید را وارد کنید (مثال: 14:00-20:00 یا 'رد کردن' для خالی کردن):", reply_markup=skip_button())
        ...
        if text == f"{EMOJIS['add_score']} اضافه کردن امتیاز":
            bot.send_message(user_id, f"{EMOJIS['info']} شناسه عددی کاربر را برای اضافه کردن امتیاز وارد کنید:", reply_markup=admin_sub_menu())
            user_states[user_id] = "admin_add_score_numeric_id"
        elif text == f"{EMOJIS['remove_score']} کم کردن امتیاز":
            bot.send_message(user_id, f"{EMOJIS['info']} شناسه عددی کاربر را برای کم کردن امتیاز وارد کنید:", reply_markup=admin_sub_menu())
        ...
        try:
            ...
            if target_user:
                ...
                bot.send_message(user_id, f"{EMOJIS['info']} چند امتیاز می‌خواهید به این کاربر اضافه کنید؟", reply_markup=admin_sub_menu())
        ...
        try:
            ...
            if target_user:
                ...
                bot.send_message(user_id, f"{EMOJIS['info']} چند امتیاز می‌خواهید از این کاربر کم کنید؟", reply_markup=admin_sub_menu())
