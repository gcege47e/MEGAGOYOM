...
def main_menu(user_id):
    ...
    keyboard.row(f\"📢 گزارش\", f\"💰 خرید مکان برتر\")
...
def handle_message(message):
    ...
    if state == "awaiting_name":
        ...
        bot.send_message(user_id, "لطفاً اگر گزارشی دارید بنویسید تا به ادمین ارسال شود:", reply_markup=back_button_only())
        ...
        if text.strip():
            ...
            keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
            keyboard.row("✅ تایید ارسال", f"{EMOJIS['back']} برگشت")
            bot.send_message(user_id, "آیا از ارسال این گزارش مطمئن هستید؟", reply_markup=keyboard)
            user_states[user_id] = "report_confirm"
        else:
            bot.send_message(user_id, f"{EMOJIS['error']} متن گزارش نمی‌تواند خالی باشد!")

    elif state == "report_confirm":
        if text == "✅ تایید ارسال":
            report_text = user_data[user_id]['report']
            user = get_user(user_id)
            profile_info = f"نام: {user[1]}\nسن: {user[2]}\nجنسیت: {user[3]}\nشناسه: {user[5]}"
            admin_msg = f"📢 گزارش جدید دریافت شد:\n\n{report_text}\n---\n{profile_info}"
