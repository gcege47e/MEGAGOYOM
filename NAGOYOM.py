...
PLACE_CATEGORIES = {
    ...
    "⚽ ورزش و سرگرمی": [
        ...
        "استخر و مجموعه ورزشی (استخر، سونا، جكوزی)",
...
def is_user_blocked_by_place_owner(user_id, place_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT blocker_id FROM blocked_users WHERE blocked_id = ? AND place_id = ?", (user_id, place_id))
    result = c.fetchone()
    conn.close()
    return result is not None

def block_user_by_place_owner(blocker_id, blocked_id, place_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT INTO blocked_users (blocker_id, blocked_id, place_id) VALUES (?, ?, ?)",
              (blocker_id, blocked_id, place_id))
    conn.commit()
    conn.close()

def unblock_user_by_place_owner(blocker_id, blocked_id, place_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("DELETE FROM blocked_users WHERE blocker_id = ? AND blocked_id = ? AND place_id = ?",
              (blocker_id, blocked_id, place_id))
    conn.commit()
    conn.close()
...
- ارتباط مستقیم با پشتیبانی
        ...
        return f"""
{EMOJIS['help']} راهنمای ربات گویم‌نما - صفحه ４/５
...
- تنظیمات سیستم
        ...
        return f"""
{EMOJIS['help']} راهنمای ربات گویم‌نما - صفحه ５/５
...
{EMOJIS['star']} امتیاز: {get_place_rating(place[0])}
            ...
            # اضافه کردن دکمه‌های نظر دادن و نظرات
            keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
            keyboard.row(f"{EMOJIS['comment']} نظر دادن", f"{EMOJIS['view']} نظرات")
            keyboard.row(f"{EMOJIS['message']} ارسال پیام به صاحب مکان")
            keyboard.row(f"{EMOJIS['back']} برگشت", f"{EMOJIS['home']} صفحه اصلی")
            
            bot.send_message(user_id, place_text, reply_markup=keyboard)
            set_user_data(user_id, "current_place_id", place[0])
    else:
...
{EMOJIS['location']} آدرس: {place[6]}
{EMOJIS['phone']} تلفن: {place[7]}
{EMOJIS['time']} ساعت کاری صاح: {place[9]}
{EMOJIS['time']} ساعت کاری عصر: {place[10]}
{EMOJIS['star']} امتیاز: {get_place_rating(place[0])}
                """
...
def handle_admin_add_vote_count(message):
    ...
    if message.text == f"{EMOJIS['back']} برگشت":
        bot.send_message(user_id, "لطفاً شناسه عددی пользователя صاحب مکان را وارد کنید:", reply_markup=back_button_only())
        set_user_state(user_id, "ADMIN_ADD_VOTE_USER_ID")
...
@bot.message_handler(func=lambda message: message.text == f"{EMOJIS['comment']} نظر دادن" and get_user_state(message.from_user.id) == "PLACES")
def handle_add_comment(message):
    user_id = message.from_user.id
    place_id = get_user_data(user_id, "current_place_id")
    
    if place_id:
        bot.send_message(user_id, "لطفاً نظر خود را برای این مکان وارد کنید:", reply_markup=back_button_only())
        set_user_state(user_id, "ADD_COMMENT")
    else:
        bot.send_message(user_id, "خطا در دریافت اطلاعات مکان.", reply_markup=get_place_menu())

@bot.message_handler(func=lambda message: get_user_state(message.from_user.id) == "ADD_COMMENT")
def handle_add_comment_text(message):
    user_id = message.from_user.id
    ...
    if message.text == f"{EMOJIS['back']} برگشت":
        ...
        if place_id:
            # ذخیره نظر
            add_comment(place_id, user_id, message.text)
            ...
            if place:
                ...
                if user:
                    comment_text = f"""
{EMOJIS['comment']} نظر جدید برای مکان شما:
...
{message.text}
                    ...
                    try:
                        bot.send_message(place[1], comment_text)
                    except:
                        pass
            ...
            bot.send_message(user_id, "خطا در ثبت نظر.", reply_markup=get_place_menu())
            set_user_state(user_id, "PLACES")

@bot.message_handler(func=lambda message: message.text == f"{EMOJIS['view']} نظرات" and get_user_state(message.from_user.id) == "PLACES")
def handle_view_comments(message):
    user_id = message.from_user.id
    place_id = get_user_data(user_id, "current_place_id")
    ...
    if place_id:
        ...
        if comments:
            for comment in comments:
                comment_text = f"""
{EMOJIS['comment']} نظر #{comment[0]}

{EMOJIS['user']} نام: {comment[5]}
{EMOJIS['info']} سن: {comment[6]}
{EMOJIS['group']} جنسیت: {comment[7]}
{EMOJIS['document']} شناسه عددی: {comment[8]}

{EMOJIS['edit']} متن نظر:
{comment[4]}
                """
                
                bot.send_message(user_id, comment_text)
        else:
            bot.send_message(user_id, "هنوز نظری برای این مکان ثبت نشده است.")
    else:
        bot.send_message(user_id, "خطا در دریافت اطلاعات مکان.", reply_markup=get_place_menu())

@bot.message_handler(func=lambda message: message.text == f"{EMOJIS['message']} ارسال پیام به صاحب مکان" and get_user_state(message.from_user.id) == "PLACES")
def handle_send_message_to_owner(message):
    user_id = message.from_user.id
    place_id = get_user_data(user_id, "current_place_id")
    ...
    if place_id:
        ...
        if place:
            ...
            if is_user_blocked_by_place_owner(user_id, place_id):
                bot.send_message(user_id, f"{EMOJIS['block']} شما توسط صاحب این مکان بلاک شده‌اید و نمی‌توانید پیام ارسال کنید.", reply_markup=get_place_menu())
                return
            
            bot.send_message(user_id, "لطفاً پیام خود را برای صاحب مکان وارد کنید:", reply_markup=back_button_only())
            set_user_state(user_id, "SEND_MESSAGE_TO_OWNER")
        else:
            bot.send_message(user_id, "خطا در دریافت اطلاعات مکان.", reply_markup=get_place_menu())
    else:
        bot.send_message(user_id, "خطا در دریافت اطلاعات مکان.", reply_markup=get_place_menu())

@bot.message_handler(func=lambda message: get_user_state(message.from_user.id) == "SEND_MESSAGE_TO_OWNER")
def handle_send_message_to_owner_text(message):
    user_id = message.from_user.id
...
@bot.message_handler(func=lambda message: message.text == f"{EMOJIS['reply']} جواب دادن" and get_user_state(message.from_user.id) == "HOME")
def handle_reply_to_message(message):
    user_id = message.from_user.id
    bot.send_message(user_id, "لطفاً پاسخ خود را وارد کنید:", reply_markup=back_button_only())
    set_user_state(user_id, "REPLY_TO_MESSAGE")

@bot.message_handler(func=lambda message: get_user_state(message.from_user.id) == "REPLY_TO_MESSAGE")
def handle_reply_to_message_text(message):
    user_id = message.from_user.id
    ...
    if message.text == f"{EMOJIS['back']} برگشت":
        ...
        # این بخش نیاز به پیاده‌سازی منطق ذخیره‌سازی پیام‌های قبلی دارد
        # برای سادگی، فقط یک پیام عمومی ارسال می‌کنیم
        bot.send_message(user_id, f"{EMOJIS['success']} پاسخ شما ارسال شد!", reply_markup=main_menu(user_id))
        set_user_state(user_id, "HOME")

@bot.message_handler(func=lambda message: message.text == f"{EMOJIS['block']} بلاک کردن" and get_user_state(message.from_user.id) == "HOME")
def handle_block_user(message):
    user_id = message.from_user.id
    # این بخش نیاز به پیاده‌سازی منطق ذخیره‌سازی کاربران بلاک شده دارد
    bot.send_message(user_id, f"{EMOJIS['success']} کاربر با موفقیت بلاک شد!", reply_markup=main_menu(user_id))

@bot.message_handler(func=lambda message: message.text == f"{EMOJIS['unblock']} رفع بلاک" and get_user_state(message.from_user.id) == "HOME")
def handle_unblock_user(message):
    user_id = message.from_user.id
    # این بخش نیاز به پیاده‌سازی منطق ذخیره‌سازی کاربران بلاک شده دارد
    bot.send_message(user_id, f"{EMOJIS['success']} کاربر با موفقیت از بلاک خارج شد!", reply_markup=main_menu(user_id))
