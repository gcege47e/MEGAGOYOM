...
def handle_message(message):
    ...
    if text == f"{EMOJIS['back']} برگشت":
        if state in ["profile_menu", "profile_view", "profile_edit_name", "profile_edit_age", "profile_edit_gender"]:
            ...
            return
                       ...
                       "admin_news_menu", "admin_news_group_content", "admin_news_group_confirm", "admin_news_user_content",
                       "admin_news_user_numeric_id", "admin_news_user_confirm"]:
            if user_id == ADMIN_ID:
    ...
    if state == "awaiting_name":
        ...
        if text == f"{EMOJIS['add']} اضافه کردن مکان":
            ...
            conn = sqlite3.connect(DB_NAME)
            c = conn.cursor()
            c.execute("SELECT * FROM places WHERE user_id = ? ORDER BY (rating_sum * 1.0 / NULLIF(rating_count, 0)) DESC, place_id DESC", (user_id,))
            places = c.fetchall()
            conn.close()
            if places:
                for place in places:
        ...
        if text.startswith("🔹"):
            ...
            if subcategory in PLACE_CATEGORIES.get(category, []):
                conn = sqlite3.connect(DB_NAME)
                c = conn.cursor()
                c.execute("SELECT places.*, users.numeric_id FROM places JOIN users ON places.user_id = users.user_id WHERE category = ? AND subcategory = ? ORDER BY (rating_sum * 1.0 / NULLIF(rating_count, 0)) DESC, place_id DESC", (category, subcategory))
                places = c.fetchall()
                conn.close()
                if places:
                    for place in places:
                ...
                bot.send_message(user_id, "انتخاب کنید:", reply_markup=back_button_only())
        ...
        query += " ORDER BY (rating_sum * 1.0 / NULLIF(rating_count, 0)) DESC, place_id DESC"
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        ...
        if text == "👥 کاربران فعال":
            ...
            bot.send_message(user_id, f"{EMOJIS['info']} عنوان مکان را برای حذف وارد کنید:", reply_markup=back_button_only())
            ...
            bot.send_message(user_id, f"{EMOJIS['info']} شناسه عددی пользова را برای رفع مسدودیت وارد کنید:", reply_markup=back_button_only())
            user_states[user_id] = "admin_unblock_user"
    elif state == "admin_news_menu":
        if text == f"{EMOJIS['group']} ارسال خبر گروهی":
            bot.send_message(user_id, f"{EMOJIS['news']} متن خبر گروهی را وارد کنید:", reply_markup=back_button_only())
            user_states[user_id] = "admin_news_group_content"
        elif text == f"{EMOJIS['user']} ارسال خبر به کاربر":
            bot.send_message(user_id, f"{EMOJIS['news']} متن خبر را برای کاربر وارد کنید:", reply_markup=back_button_only())
            user_states[user_id] = "admin_news_user_content"
    elif state == "admin_news_group_content":
        if text.strip():
            user_data[user_id] = {'news_content': text}
            keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
            keyboard.row("بله", "خیر")
            keyboard.row(f"{EMOJIS['back']} برگشت")
            bot.send_message(user_id, f"{EMOJIS['info']} آیا مطمئن هستید که می‌خواهید این خبر را به همه کاربران ارسال کنید؟\n\n{text}", reply_markup=keyboard)
            user_states[user_id] = "admin_news_group_confirm"
        else:
            bot.send_message(user_id, f"{EMOJIS['error']} متن خبر نمی‌تواند خالی باشد!", reply_markup=back_button_only())
    elif state == "admin_news_group_confirm":
        if text == "بله":
        ...
        if text.strip():
            user_data[user_id] = {'news_content': text}
            bot.send_message(user_id, f"{EMOJIS['info']} شناسه عددی کاربر مورد نظر را وارد کنید:", reply_markup=back_button_only())
        ...
        try:
            ...
            target_user = get_user_by_numeric_id(numeric_id)
            if target_user:
                user_data[user_id]['target_numeric_id'] = numeric_id
                user_data[user_id]['target_user_id'] = target_user[0]
                user_data[user_id]['target_name'] = target_user[1]
                user_data[user_id]['target_age'] = target_user[2]
                user_data[user_id]['target_gender'] = target_user[3]
                ...
                msg += f"📝 خبر:\n{user_data[user_id]['news_content']}"
        ...
        if text.strip():
            ...
            bot.send_message(user_id, f"{EMOJIS['error']} عنوان مکان نمی‌تواند خالی باشد!", reply_markup=back_button_only())
        ...
        if text.startswith("🔹"):
            ...
            if subcategory in PLACE_CATEGORIES.get(category, []):
                ...
                bot.send_message(user_id, f"{EMOJIS['info']} شناسه عددی کاربر مالک مکان را وارد کنید:", reply_markup=back_button_only())
