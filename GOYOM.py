...
def get_gender(message):
    ...
    if gender in ["پسر", "دختر"]:
        ...
        age = data['reg_age']
        cursor.execute("INSERT INTO users (user_id, username, age, gender) VALUES (?, ?, ?, ?)",
...
def create_poll_publish(message):
    ...
    cursor.execute("DELETE FROM user_states WHERE user_id = ?", (user_id,))
    conn.commit()
    bot.send_message(user_id, "نظرسنجی ارسال شد.", reply_markup=get_admin_keyboard())
...
def show_ads_in_sub_cat(message):
    ...
    if sub_cat == "بازگشت به دسته‌های اصلی":
...
def show_selected_ad(message):
    ...
    sub_cat = parts[3]
    title = message.text
    if title == "بازگشت به زیر دسته‌ها":
    ...
    ad = cursor.fetchone()
    if ad:
        ad_id, user_id, title, description, _, _, photo_id, contact = ad
...
def get_ad_photo(message):
    ...
    bot.send_message(user_id, "عکس آگهی را ارسال کنید (یا بنویسید 'بدون عکس'):")
...
@bot.message_handler(func=lambda message: get_user_state(message.from_user.id)[0] == 'awaiting_ad_photo' and message.text == 'بدون عکس')
def process_ad_no_photo(message):
    user_id = message.from_user.id
    state, data = get_user_state(user_id)
    cursor.execute("INSERT INTO ads (user_id, title, description, category_main, category_sub, contact) VALUES (?, ?, ?, ?, ?, ?)",
                  (user_id, data['title'], data['description'], data['category_main'], data['category_sub'], data['contact']))
    conn.commit()
    cursor.execute("DELETE FROM user_states WHERE user_id = ?", (user_id,))
    conn.commit()
    update_user_points(user_id, 10)
    bot.send_message(user_id, "آگهی بدون عکس ثبت شد و 10 امتیاز گرفتید!", reply_markup=get_main_keyboard())
...
def get_new_ad_description(message):
    ...
    if not new_description:
        bot.send_message(user_id, "توضیحات معتبر وارد کنید:")
        return
    cursor.execute("UPDATE ads SET title = ?, description = ? WHERE id = ? AND user_id = ?",
                  (new_title, new_description, ad_id, user_id))
    conn.commit()
    cursor.execute("DELETE FROM user_states WHERE user_id = ?", (user_id,))
    conn.commit()
...
def manage_shops_start(message):
    ...
    if shops:
        ...
        for shop in shops:
            keyboard.add(shop[1])
        keyboard.add("⬅️ بازگشت به پنل مدیریت")
        bot.send_message(message.from_user.id, "مکان‌های شما:", reply_markup=keyboard)
    else:
        bot.send_message(message.from_user.id, "مغازه‌ای ثبت نشده است.", reply_markup=get_admin_keyboard())
...
def show_admin_shop_details(message):
    ...
    if shop:
        shop_id, title, description, address, category, photo_id, contact, _, seller_username, seller_age, seller_gender = shop
...
def get_seller_gender(message):
    ...
    if gender in ["پسر", "دختر"]:
        ...
        bot.send_message(user_id, "عکس مکان را ارسال کنید (یا بنویسید 'بدون عکس'):", reply_markup=get_admin_sub_menu_keyboard())
    else:
...
def get_shop_photo(message):
    ...
    cursor.execute("INSERT INTO shops (title, description, address, category, photo_id, contact, admin_id, seller_username, seller_age, seller_gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                  (data['title'], data['description'], data['address'], data['category'], photo_id, data['contact'], user_id, data['seller_username'], data['seller_age'], data['seller_gender']))
    conn.commit()
    cursor.execute("DELETE FROM user_states WHERE user_id = ?", (user_id,))
    conn.commit()
    bot.send_message(user_id, "مکان ثبت شد.", reply_markup=get_admin_keyboard())

@bot.message_handler(func=lambda message: get_user_state(message.from_user.id)[0] == 'awaiting_shop_photo' and message.text == 'بدون عکس' and message.from_user.id == ADMIN_ID)
def get_shop_no_photo(message):
    user_id = message.from_user.id
    state, data = get_user_state(user_id)
    cursor.execute("INSERT INTO shops (title, description, address, category, contact, admin_id, seller_username, seller_age, seller_gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                  (data['title'], data['description'], data['address'], data['category'], data['contact'], user_id, data['seller_username'], data['seller_age'], data['seller_gender']))
    conn.commit()
    cursor.execute("DELETE FROM user_states WHERE user_id = ?", (user_id,))
    conn.commit()
    bot.send_message(user_id, "مکان بدون عکس ثبت شد.", reply_markup=get_admin_keyboard())
...
def show_user_shop_details(message):
    ...
    if shop:
        ...
        if photo_id:
            bot.send_photo(user_id, photo_id, caption=response, reply_markup=keyboard)
        else:
            bot.send_message(user_id, response, reply_markup=keyboard)
...
def get_new_shop_description(message):
    ...
    if not new_description:
        bot.send_message(user_id, "توضیحات معتبر وارد کنید:")
        return
    cursor.execute("UPDATE shops SET title = ?, description = ? WHERE id = ? AND admin_id = ?",
                  (new_title, new_description, shop_id, user_id))
    conn.commit()
    cursor.execute("DELETE FROM user_states WHERE user_id = ?", (user_id,))
    conn.commit()
...
def process_rating(message):
    ...
    try:
        ...
        if 0 <= rating <= 10:
            cursor.execute("INSERT OR REPLACE INTO shop_ratings (shop_id, user_id, rating) VALUES (?, ?, ?)",
